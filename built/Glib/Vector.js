...
class Vector {
    ...
    //Vector operand
    plus(vec) {
        if (vec.Dims !== this.Dims)
            throw new Error('Vector dimensionality must match');
    ...
    cross(vec) {
        if (vec.Dims !== 3 || this.Dims !== 3)
            throw new Error('Vector dimensionality must match to 3');
        return new Vector(this.Values[1] * vec.Values[2] - this.Values[2] * vec.Values[1], this.Values[2] * vec.Values[0] - this.Values[0] * vec.Values[2], this.Values[0] * vec.Values[1] - this.Values[1] * vec.Values[0]);
    }
    angle(that) {
