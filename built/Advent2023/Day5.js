...
main_1.Data[0].slice(7).toIntList(10, ' ')
    .map((seed) => {
    steps1.forEach(s => {
        const step = s.SplitLines().slice(1).map(r => r.toIntList(10, ' '));
        step.ForEach(r => {
...
//part 2
// [INCLUSIVE, EXCLUSIVE]
const mappings = main_1.DataFull.Split2Lines().slice(1).map(s => s.SplitLines().slice(1).map(r => r.toIntList(10, ' '))
    .map(r => [r[1], r[1] + r[2], r[0] - r[1]] //rangeLo, rangeHi, offset
).sort((a, b) => a[0] - b[0]));
let ranges = [];
const a = main_1.Data[0].slice(7).toIntList(10, ' ');
