...
exports.Advent2021 = void 0;
const BigMap_1 = require("../Glib/BigMap");
...
const Array_1 = require("../Glib/Array");
...
class Advent2021 {
    ...
    static Day3() {
        let oxygen = Data.Copy(), co2 = Data.Copy();
        Data.ReduceFilter((n, i) => n[i] === Data.map(x => x[i])
    ...
    static Day4() {
        const d = DataFull.split('\n\n'), randoms = d[0].split(',').toIntArray(), boards = d.slice(1).map(b => {
            return {
                bingo: false,
                board: b.SplitLines().map(l => l.replaceAll('  ', ' ').trim().split(' ')
                    .toIntArray()
        ...
        for (const rand of randoms) {
            for (const board of boards.filter(b => !b.bingo)) {
                //mark tiles
                board.board.forEach(row => row.forEach(t => (t.value === rand).IsTrue(() => t.marked = true)));
                //check bingos
                for (let i = 0; i < 5; i++) {
                    const row = board.board[i], col = board.board.map(row => row[i]);
    ...
    static Day5() {
        ...
        for (const p of d)
            if (p.x1 === p.x2) //vertical
                (0, Array_1.Range)(p.y1, p.y2).forEach(n => map.IncrementOrCreate2D(n, p.x1));
            else if (p.y1 === p.y2) //horizontal
                (0, Array_1.Range)(p.x1, p.x2).forEach(n => map.IncrementOrCreate2D(p.y1, n));
            else //diagonal
                for (let i = 0; i <= Math.abs(p.x1 - p.x2); i++)
                    map.IncrementOrCreate2D(p.y1 + (p.y1 > p.y2 ? -i : i), p.x1 + (p.x1 > p.x2 ? -i : i));
        const len = map.reduce((p, c) => c.length > p ? c.length : p, 0);
        map.FillEmpty([]).forEach(r => r.FillEmpty(0, len)
    ...
    static Day6_revised() {
        const school = Array(9).fill(BigInt(0));
    ...
    static Day7() {
        ...
        (0, Array_1.Range)(0, Math.max(...d)) //could binary search
            .map(i => d.map(crab => {
            const n = Math.abs(crab - i);
            return n * (n + 1) / 2;
        }).Sum())
            .reduce((prev, cur, i) => prev[0] > cur ? [cur, i] : prev, [Number.MAX_VALUE, 0]).Log();
    }
    static Day8() {
        const d = Data.map(display => display.split(' | ')
            .map(d => d.split(' ').map(a => a.toArray()))), sevenSegs = {
            'ABCEFG': 0,
            'CF': 1,
            'ACDEG': 2,
            'ACDFG': 3,
            ...
            'ABCDEFG': 8,
            'ABCDFG': 9,
        }, validSegments = Object.keys(sevenSegs), wires = 'abcdefg'.toArray(), perms = 'ABCDEFG'.toArray().Permutations(), mapDigit = (d, map) => d.map(wire => map[wire]).sort(Sort_1.Sorts.Alphabetical).join('');
        d.reduce((count, cur) => {
            const [signals, segments] = cur;
            for (const perm of perms) { //loop through all wire => segment mappings
                const map = perm.map((val, i) => [wires[i], val]).toObject();
                if (signals.every(digit => validSegments.includes(mapDigit(digit, map))))
                    return count + segments.map(digit => //count the output of those which can decode all signals into digits
                     sevenSegs[mapDigit(digit, map)]).join('').toInt();
            }
            throw new Error('Could not find successful permutation');
        }, 0).Log();
    }
    static Day9() {
        const d = XY_1.Array2D.fromArray(Data.map(d => d.toArray().toIntArray())), basins = [];
        d.forEach((tile, xy) => {
        ...
        basins.map(basin => {
            const b = new Set().add(basin.toString());
            for (const bb of b) {
                d.Neighbours(XY_1.XY.fromString(bb)).forEach(t => (t[1] < 9).IsTrue(() => b.add(t[0].toString())));
            }
            return b.size;
        }).sort(Sort_1.Sorts.GreatestFirst).slice(0, 3).Product().Log();
    }
    static Day10() {
        const invertBracket = {
            '(': ')',
    ...
    static Day11() {
        ...
        let s = XY_1.Array2D.fromArray(Data.map(l => l.toArray().toIntArray())).map(e => ({ energy: e, flashed: false }));
        for (let i = 1;; i++) {
            s.forEach(octopus => {
    ...
    static Day12() {
        let d = Data.map(l => l.split('-')), dFull = d.flatMap(l => [l, l.Copy().reverse()]), adjacents = d.flat().Uniques().Log().map(node => [node, dFull.filter(edge => edge[0] === node).map(e => e[1]).filter(e => e != 'start')]).toObject().Log();
        let paths = 0, step = (path) => {
    ...
    static Day13() {
        const [ps, fs] = DataFull.split('\n\n').map(n => n.SplitLines()), pairs = ps.map(d => d.split(',').toIntArray()), folds = fs.map(f => f.split(' ').at(-1).split('='));
        let paper = new XY_1.Array2D(new XY_1.XY(pairs.map(p => p[0]).Max() + 1, pairs.map(p => p[1]).Max() + 1));
        pairs.forEach(pair => paper.set(new XY_1.XY(pair[0], pair[1]), true));
        for (const fold of folds) {
            const newPaper = new XY_1.Array2D(new XY_1.XY(fold[0] === 'x' ? (paper.Size.X - 1) / 2 : paper.Size.X, fold[0] === 'y' ? (paper.Size.Y - 1) / 2 : paper.Size.Y));
            paper.forEach((dot, xy) => {
                const foldPos = fold[1].toInt();
                if (dot) {
                    newPaper.set(fold[0] === 'x'
                        ? (xy.X > foldPos ? new XY_1.XY(foldPos - (xy.X - foldPos), xy.Y) : xy)
                        : (xy.Y > foldPos ? new XY_1.XY(xy.X, foldPos - (xy.Y - foldPos)) : xy), dot);
                }
            });
            paper = newPaper;
        }
    ...
    static Day14() {
        const [template, r] = DataFull.split('\n\n').map(p => p.SplitLines()), polymer = template[0].Log().toArray(), rules = r.map(rule => rule.split(' -> ')).toObject().Log(), freqs = {};
        polymer.forEach(e => {
            if (freqs[e])
                freqs[e]++;
            else
    ...
    static Day14_revised() {
        const [template, r] = DataFull.split('\n\n').map(p => p.SplitLines()), poly = template[0].Log().toArray(), rules = r.map(rule => rule.split(' -> ')).toObject().Log(), polymer = new LinkedList_1.LinkedList(), freqs = {};
        poly.forEach(char => {
            polymer.Push(new LinkedList_1.LinkedNode(char));
        });
        poly.forEach(e => {
            if (freqs[e])
                freqs[e]++;
        ...
        for (let i = 0; i < 40; i++) {
            ...
            while (node?.Next) {
                ...
                node.InsertAfter(new LinkedList_1.LinkedNode(e));
                node = node.Next.Next;
                if (freqs[e])
                    freqs[e]++;
                else
    ...
    static Day14_revised_revised() {
        const [template, r] = DataFull.split('\n\n').map(p => p.SplitLines()), rules = r.map(rule => rule.split(' -> ')).toObject(), emptyFreqs = rules.Keys().map(v => [v, 0]), letterfreqs = {};
        let freqs = emptyFreqs.toObject();
        ...
        for (let i = 0; i < 40; i++) {
            const newFreqs = emptyFreqs.toObject();
    ...
    static Day15() {
        const d = XY_1.Array2D.fromArray(Data.map(l => l.toArray().toIntArray())), lastE = d.Size.minus(1);
        let smallestPath = Number.MAX_VALUE, steps = 0;
        const bestRoute = (xy, score = 0) => {
            ...
            const xp = xy.plus(1, 0), yp = xy.plus(0, 1), xVal = d.get(xp), yVal = d.get(yp);
            if (xVal && yVal) {
                if (xVal > yVal) {
        ...
        d.Log();
        bestRoute(new XY_1.XY);
        console.log(steps, 'steps');
    }
    static Day15_revised() {
        const d = XY_1.Array2D.fromArray(Data.map(l => l.toArray().toIntArray())), bestScore = new XY_1.Array2D(d.Size), lastE = d.Size.minus(1);
        ...
        [bestScore.get(new XY_1.XY(1, 0)),
            bestScore.get(new XY_1.XY(0, 1))].Min()?.Log();
    }
    static Day15_revised_revised() {
        const d = XY_1.Array2D.fromArray(Data.map(l => l.toArray().toIntArray())), bestScore = new XY_1.Array2D(d.Size), lastE = d.Size.minus(1);
        bestScore.set(new XY_1.XY, 0);
        d.Log();
        // for (let i = 1; ; i++) {
        //     for (let j = 0; j < i + 1; j++) {
    ...
    static Day15_revised_revised_revised() {
        const d = XY_1.Array2D.fromArray(Data.map(l => l.toArray().toIntArray().map(e => ({ weight: e, visited: false, distance: Number.POSITIVE_INFINITY }))));
        const dd = new XY_1.Array2D(d.Size.times(5)), lastE = dd.Size.minus(1);
        d.forEach((val, xy) => {
            if (!val)
                return;
            for (let i = 0;; i++) {
                for (let j = 0; j < i + 1; j++) {
                    const newxy = new XY_1.XY(i - j, j);
                    if (newxy.IsLessBoth(new XY_1.XY(5))) {
                        dd.set(newxy.times(d.Size).plus(xy), val.Copy());
                        if (newxy.EQ(new XY_1.XY(4)))
                            return;
                    }
                }
        ...
        dd.get(new XY_1.XY).distance = 0;
        let currentNode = new XY_1.XY;
        for (let i = 0;; i++) {
        ...
        class Packet {
            ...
            constructor(bits, position = 0) {
                ...
                if (this.TypeId === 4) {
                    ...
                    //operator
                    const lenType = bits.charAt(this.Position++).toInt(), lenTypeSize = lenType === 0 ? 15 : 11, subPacketSize = bits.slice(this.Position, this.Position += lenTypeSize).toInt(2);
                    let p;
                    do {
            ...
            SumVersions = () => this.Version + this.SubPackets?.reduce((a, v) => a + v.SumVersions(), 0) ?? 0;
            Operate() {
    ...
    static Day17() {
        const t = Data[0].split(': ')[1].split(', ').map(p => XY_1.XY.fromTuple(p.slice(2).split('..').toIntArray())), target = [new XY_1.XY(t[0].Least, t[1].Least), new XY_1.XY(t[0].Greatest, t[1].Greatest)];
        const IsValid = (velocity) => {
            const position = new XY_1.XY;
            while (position.X < target[1].X && position.Y > target[0].Y) {
                position.plusEQ(velocity);
                velocity.plusEQ(velocity.X > 0 ? -1 : velocity.X === 0 ? 0 : 1, -1);
                if (position.WithinBounds(...target))
                    return true;
            }
            return false;
        };
        const xy = new XY_1.XY;
        while (!xy.X.SumOfLess().InRangeEq(target[0].X, target[1].X))
            xy.X++;
        while (xy.Y + 1 < Math.abs(target[0].Y))
            xy.Y++;
        //x must be in rangeEQ(x, tuple[1].X)
        //y must be in rangeEQ(tuple[0].Y, y)
        new XY_1.XY(target[1].X, xy.Y).CountCombinations(xy => IsValid(xy), new XY_1.XY(xy.X, target[0].Y)).Log();
    }
    static Day18() {
        class Snail {
            Tuple;
            constructor(tupleNumber) {
                if (typeof tupleNumber === 'string') {
                    let snailNumber = tupleNumber;
                    let parseDepth = 0, delimiterPos;
                    ...
                    if (!delimiterPos)
                        throw new Error('Could not find delimiter in ' + snailNumber);
                    const tuple = [snailNumber.substring(0, delimiterPos),
                        snailNumber.substring(delimiterPos + 1)];
                    this.Tuple = tuple.map(t => !isNaN(t.toInt()) ? t.toInt() : new Snail(t));
                }
                else {
            ...
            AddFirst(n) {
                if (typeof this.Tuple[0] === 'number')
                    ...
                    this.Tuple[0].AddFirst(n);
            }
            AddLast(n) {
                if (typeof this.Tuple[1] === 'number')
                    this.Tuple[1] += n;
                else
                    this.Tuple[1].AddLast(n);
            }
            Magnitude() {
            ...
            TryExplode(depth = 0) {
                if (depth >= 3) {
                    if (typeof this.Tuple[0] !== 'number') {
                        const addNumbers = this.Tuple[0].Tuple;
                        if (typeof this.Tuple[1] === 'number')
                            this.Tuple[1] += addNumbers[1];
                        else
                            this.Tuple[1].AddFirst(addNumbers[1]);
                        this.Tuple[0] = 0;
                        return [addNumbers[0], 0, true];
                    }
                    if (typeof this.Tuple[1] !== 'number') {
                        const addNumbers = this.Tuple[1].Tuple;
                ...
                if (typeof this.Tuple[1] !== 'number') {
                    ...
                    if (addNumbers[0] > 0) {
                        if (typeof this.Tuple[0] === 'number')
                            ...
                            this.Tuple[0].AddLast(addNumbers[0]);
                        return [0, 0, true];
                    }
                    if (addNumbers[1] > 0)
                        return addNumbers;
            ...
            Reduce() {
                while (true) {
                    if (this.TryExplode()[2])
                        continue;
                    else if (this.TrySplit())
                        continue;
                    break;
                }
            }
    ...
    static Day19() {
        class Scanner {
            ...
            MergePoints(translation = new XYZ_1.XYZ) {
                let list = this.Orientations[this.AbsoluteOrientation].Beacons.map(p => p.plus(translation));
                for (const n of this.Neighbours) {
                    list = list.concat(n[0].MergePoints(n[1].plus(translation)));
                }
                return list;
            }
            Translations(translation = new XYZ_1.XYZ) {
        ...
        const scanners = DataFull.split('\n\n').map((scannerData, i) => new Scanner(scannerData.SplitLines().slice(1).map(l => XYZ_1.XYZ.fromTuple(l.split(',').toIntArray())), i));
        const overlap = 12;
        // d[0].Orientations[0].Log().Overlap(d[1].Orientations[0].Log()).Log()
        const head = scanners[0];
        head.AbsoluteOrientation = 0;
        const findNeighbors = (s, possibleNeighbors) => {
    ...
    static Day20() {
        ...
        let image = XY_1.Array2D.fromArray(Data.slice(2).map(l => l.toArray().map(char => char === '#')));
        image = new XY_1.Array2D(image.Size.plus(2)).map((_, xy) => image.get(xy.minus(1)));
        for (let i = 0; i < 8; i++) {
            image = new XY_1.Array2D(image.Size.plus(2)).map((_, xy) => algorithm[xy.minus(1).Neighbourhood(true).map(xy => image.get(xy) ? '1' : '0').join('').toInt(2)]);
            if (i % 2 === 1) {
                image.Array = [
    ...
    static Day21_2() {
        const r = (0, Array_1.Range)(3, 9).map(v => [v, 0]).toObject();
        ...
        const rolls = r.Entries().map(e => [e[0].toInt(), BigInt(e[1])]);
        rolls.Log();
        const wins = [0n, 0n];
        class Game {
            Position1;
            ...
            Position2;
            Score2;
            constructor(Position1, Score1, Position2, Score2) {
            ...
            static fromString(s) {
                const ss = s.split(','), sss = ss.toIntArray();
                ...
                for (const [roll1, rollf1] of rolls) {
                    ...
                    if (g.Move(1, roll1))
                        ...
                        for (const [roll2, rollf2] of rolls) {
                            const gg = g.Copy(), f = freq * rollf1 * rollf2;
                            if (gg.Move(2, roll2))
                                wins[1] += f;
                            else
                                newList.IncrementOrCreate(gg.toString(), f);
                        }
                    }
                }
            });
    ...
    static Day22() {
        ...
        for (const l of Data) {
            ...
            const min = XYZ_1.XYZ.fromTuple(pos.map(a => a[0] < -50 ? -50 : a[0])), max = XYZ_1.XYZ.fromTuple(pos.map(a => a[1] > 50 ? 50 : a[1]));
            if (min.toArray().some((v, i) => v > max.toArray()[i]))
                continue;
            max.foreachCombination(xyz => {
                if (xyz.IsGreaterEQAll(new XYZ_1.XYZ(-50)) && xyz.IsLessEQAll(new XYZ_1.XYZ(50))) {
                    if (instr === 'on') {
    ...
    static Day22_2() {
        class Cuboid {
            ...
            MaxZ;
            Vol0;
            constructor(MinX, MaxX, MinY, MaxY, MinZ, MaxZ, Vol0) {
                this.MinX = MinX;
                this.MaxX = MaxX;
                this.MinY = MinY;
                this.MaxY = MaxY;
            ...
            Intersect(c) {
                let cc = new Cuboid([this.MinX, c.MinX].Max(), [this.MaxX, c.MaxX].Min(), [this.MinY, c.MinY].Max(), [this.MaxY, c.MaxY].Min(), [this.MinZ, c.MinZ].Max(), [this.MaxZ, c.MaxZ].Min());
                if (cc.MaxX < cc.MinX || cc.MaxY < cc.MinY || cc.MaxZ < cc.MinZ)
                    return new Cuboid(0, 0, 0, 0, 0, 0, true);
                return cc;
            }
            Volume() {
                if (this.Vol0)
        ...
        class Instruction {
            ...
            constructor(instr) {
                ...
                this.On = type === 'on';
                const pos = range.split(',').flatMap(c => c.slice(2).split('..').toIntArray());
                this.Cuboid = new Cuboid(...pos);
            }
        }
        function HowManyOn(insts, cuboid) {
            const instr = insts.at(-1);
            if (insts.length === 1) {
                if (!instr.On)
                    return 0;
                else
                    return instr.Cuboid.Intersect(cuboid).Volume();
            }
            const allOn = HowManyOn(insts.slice(0, -1), cuboid), intersection = cuboid.Intersect(instr.Cuboid);
        ...
        const reactorCuboid = is.map(i => i.Cuboid).reduce((c, cc) => {
            return new Cuboid([c.MinX, cc.MinX].Min(), [c.MaxX, cc.MaxX].Max(), [c.MinY, cc.MinY].Min(), [c.MaxY, cc.MaxY].Max(), [c.MinZ, cc.MinZ].Min(), [c.MaxZ, cc.MaxZ].Max());
        });
        reactorCuboid.Log();
    ...
    static Day23() {
        class Tile {
            ...
            Occupant;
            AtDest;
            constructor(Home, Occupant, AtDest = false) {
                this.Home = Home;
        ...
        class Board {
            ...
            NextMoves = [];
            Parent;
            constructor(tiles, Weight, parent) {
                this.Weight = Weight;
                this.Tiles = tiles;
                this.Parent = parent;
            }
                ...
                for (let from = 0; from < this.Tiles.length; from++) {
                    if (this.Tiles[from].Occupant) {
                        for (let to = 0; to < this.Tiles.length; to++) {
                            ...
                            if (this.IsObstructed(path))
                                continue;
                            const b = new Board(this.Tiles.map(t => t.Copy()), this.Weight + path.length * this.Tiles[from].Weight, this);
            ...
            FindPath(from, to) {
                ...
                if (!p)
                    throw new Error(`could not find path (${from} => ${to})`);
                return p;
            }
            IsValidMove(from, to) {
                if (from === to)
                    return false;
                if (from < 0 || from >= this.Tiles.length || to < 0 || to >= this.Tiles.length)
                    throw new Error('Bad move'); // invalid parameters
                if (this.Tiles[to].Occupant)
                    return false; // destination occupied
                if (this.Tiles[from].AtDest)
                    return false;
                if (!this.Tiles[from].Occupant)
                    return false; // nobody to move
                if ([2, 4, 6, 8].includes(to))
                    return false; // rule 1
                const room = this.rooms[to];
                if (room) {
                    //going into room
                    if (this.Tiles[to].Home !== this.Tiles[from].Occupant ||
                        room.map(t => this.Tiles[t]).some(t => t.Occupant && t.Occupant !== t.Home))
                        return false; // rule 2
                    else {
                        //home room && right people
                        //ensure deepest in room
                        if (!this.Tiles[room[1]].Occupant && to === room[0])
                            return false;
                    }
                }
                if (from <= 10 && !room)
            ...
            Log() {
                const t = (i) => this.Tiles[i].Occupant ?? '.';
                console.log(`#############`);
        ...
        //     ], 0)
        const headBoard = new Board([new Tile, new Tile, new Tile, new Tile, new Tile, new Tile, new Tile, new Tile, new Tile, new Tile, new Tile,
    ...
    static Day24() {
        ...
        function Emu2(input) {
            ...
            for (const [step, w] of steps.map((s, i) => [s, input[i]])) {
        ...
        const memo = new BigMap_1.BigMap();
        function nextZ(step, w, z) {
            const s = steps[step];
            let x = z % 26 + s.B === w ? 0 : 1;
            return (Math.floor(z / s.A) * (25 * x + 1)) + ((w + s.C) * x);
        }
        function AllValidSuffix(step, z) {
            if (step === 14)
                return z === 0 ? [] : null;
    ...
    static Day25() {
        let d = XY_1.Array2D.fromArray(Data.map(l => l.toArray().map(t => t === '.' ? undefined : t)));
        function HalfStep(d, offset, char) {
            const newD = new XY_1.Array2D(d.Size);
            d.forEach((t, xy) => {
                if (!t)
                    return;
        ...
        for (let i = 0;; i++) {
            ...
            i.Log();
            // d.Log()
            let newD = HalfStep(HalfStep(d, new XY_1.XY(1, 0), 'v'), new XY_1.XY(0, 1), '>');
            if (newD.some((t, xy) => t !== d.get(xy)))
                again = true;
            d = newD;
            if (again)
                continue;
