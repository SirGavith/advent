...
//part 1
const map: {[t: string]: string} = { T: '10', J: '11', Q: '12', K: '13', A: '14' }
Data.map(l => l.split(' ')).map(([cards,b]) => [
        b.toInt(), 
        cards.toArray().Frequencies().sort((i, j) => j[1] - i[1]).map(c => c[1]).join('').padEnd(5, '0').toInt(),
        cards.toArray().map(char => char.ReplaceMap(map).toInt())] as const)
...
//part 2
const map2: { [t: string]: string } = { T: '10', J: '1', Q: '12', K: '13', A: '14' }

Data.map(l => l.split(' ')).map(l => [l[0], l[1].toInt()] as const)
    .sort(([a, _], [b, __]) => {
        // console.log()
