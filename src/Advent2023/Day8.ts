...
const map = new Map(Data.slice(2).map(r => {
    const l = r.split(' = ')
    return [l[0], l[1].ReplaceMap({'(':'',')':''}).split(', ')] as [string,[string,string]]
}));
...
let currentNodes: string[] = Data.slice(2).map(l => l.slice(0,3)).filter(l => l.endsWith('A'))
// bf approach
// let j = 0
// for (; ; j++) {
